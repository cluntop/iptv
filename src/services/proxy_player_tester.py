...
class ProxyPlayerTester:
    ...
    async def test_http_proxy_play(
        self,
        proxy_host: str,
        proxy_port: int,
        source_url: str,
    ...
    async def test_udpxy_play(
        self, proxy_host: str, proxy_port: int, source_url: str, test_duration: int = 10
    ) -> ProxyPlayResult:
        result = ProxyPlayResult(
            proxy_host=proxy_host,
            proxy_port=proxy_port,
...
class InternalSourceTester:
    ...
    async def test_internal_source_with_proxy(
        self,
    ...
    async def test_source_with_multiple_proxies(
        self,
        source: InternalSource,
        ...
        return await self.player_tester.batch_test_proxies(
            proxy_list=proxies, source_url=source.url, max_concurrent=max_concurrent
        )

    async def find_best_proxy_for_source(
        self, source: InternalSource, proxies: List[Dict[str, Any]]
    ) -> Optional[ProxyPlayResult]:
    ...
    async def test_multiple_sources_with_proxy(
        self,
