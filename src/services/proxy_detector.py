...
class ProxyDetector:
    ...
    async def _create_session_with_proxy(self, proxy: ProxyInfo) -> aiohttp.ClientSession:
        ...
        if proxy.protocol.lower() == 'socks5':
            try:
                ...
                return aiohttp.ClientSession(connector=connector, timeout=aiohttp.ClientTimeout(total=self.timeout))
            except ImportError:
    ...
    async def check_proxy_full(self, proxy: ProxyInfo) -> ProxyCheckResult:
        async with self.semaphore:
            ...
            logger.info(f"Proxy {proxy} check completed: valid={result.is_valid}, latency={result.latency_ms}ms, exit_ip={result.exit_ip}")
    ...
    async def check_proxies_batch(self, proxies: List[ProxyInfo]) -> List[ProxyCheckResult]:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                valid_results.append(ProxyCheckResult(
                    proxy=proxies[i],
                    is_valid=False,
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', newline='') as f:
                ...
                if header:
                    ...
                    host_idx = next((i for i, h in enumerate(header) if h in ['host', 'ip', 'proxy']), None)
                    port_idx = next((i for i, h in enumerate(header) if h == 'port'), None)
                    protocol_idx = next((i for i, h in enumerate(header) if h in ['protocol', 'type']), None)
                    username_idx = next((i for i, h in enumerate(header) if h in ['username', 'user']), None)
                    password_idx = next((i for i, h in enumerate(header) if h in ['password', 'pass']), None)
                        ...
                        for row in reader:
                            if len(row) > max(host_idx, port_idx):
                                proxy = ProxyInfo(
                                    ...
                                    protocol=row[protocol_idx] if protocol_idx and len(row) > protocol_idx else 'http',
...
class ProxyService:
    ...
    def check_single_proxy(self, host: str, port: int, protocol: str = 'http',
                           username: str = None, password: str = None) -> ProxyCheckResult:
        proxy = ProxyInfo(
            host=host,
            port=port,
            protocol=protocol,
    ...
    def get_valid_proxies(self, results: List[ProxyCheckResult], 
                          min_latency: float = 0,
                          require_anonymous: bool = False) -> List[ProxyCheckResult]:
        valid = []
    ...
    def export_results(self, results: List[ProxyCheckResult], 
                       output_path: str, format: str = 'txt') -> bool:
        try:
            ...
            if format.lower() == 'csv':
                with open(path, 'w', encoding='utf-8', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        'proxy', 'valid', 'latency_ms', 'exit_ip', 
                        'country', 'region', 'city', 'isp', 'asn',
                        'anonymous', 'high_anonymous', 'error'
                    ])
