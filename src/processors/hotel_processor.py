...
from ..database import Hotel, HotelModel, Channel, ChannelModel
from ..utils import get_logger, VideoTools, StringTools, NetworkTools
...
class HotelProcessor:
    ...
    def scan_hotel_network(self, base_ip: str, port: int, 
                           thread_count: int = 6) -> List[Tuple[str, int]]:
    ...
    def process_hotel_channels(self, hotel: Dict[str, Any], 
                               categories: List[Any], sign: int = 1) -> List[Channel]:
        channels = []
        ip = hotel['ip']
        port = hotel['port']
    ...
    def update_hotel_status(self, ip: str, status: int, 
                            count: int = 0, name: str = None) -> bool:
        try:
